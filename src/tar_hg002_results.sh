...
svafotate=(
    ...
    "$(realpath ../results/2025_12-hg002_cmrg-svafotate/svafotate_hg002_cmrg.times)"
)

needlr=(
    "$(realpath ../results/2026_01-hg002-needlr/hg002_needlr_popfreqs.txt)"
    "$(realpath ../results/2026_01-hg002-needlr/needLR_run_time.tsv)"
)
...
(
    ...
    cp "${svafotate[@]}" .
    cp "${needlr[@]}" .
    tar -czvf hg002_cmrg_stix_lr_svafotate_needlr_results.tar.gz ./*
)
mv $tmp/hg002_cmrg_stix_lr_svafotate_needlr_results.tar.gz $outdir
echo "# tarball created at $outdir/hg002_cmrg_stix_lr_svafotate_needlr_results.tar.gz"
